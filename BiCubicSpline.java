...
public class BiCubicSpline{
    	...
    	public BiCubicSpline(double[] x1, double[] x2, double[][] y){
        	...
        	this.xMin[0] = CubicSpline.minimum(this.x1);
        	this.xMax[0] = CubicSpline.maximum(this.x1);
        	for(int j=0; j<this.mPoints; j++){
            		this.x2[j]=x2[j];
        	}
        	this.xMin[1] = CubicSpline.minimum(this.x2);
        	this.xMax[1] = CubicSpline.maximum(this.x2);
        	for(int i =0; i<this.nPoints; i++){
            		for(int j=0; j<this.mPoints; j++){
                		this.y[i][j]=y[i][j];
            		}
