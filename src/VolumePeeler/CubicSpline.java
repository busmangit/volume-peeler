...
public class CubicSpline{
    	...
    	public void orderPoints(){
    	    ...
    	    this.selectionSort(this.x, dummy, this.newAndOldIndices);
    	    // Sort x into ascending order and make y match the new order storing both new x and new y
    	    this.selectionSort(this.x, this.y, this.x, this.y);

    	    // Minimum and maximum values and range
    	    this.xMin = this.minimum(this.x);
    	    this.xMax = this.maximum(this.x);
    	    range = xMax - xMin;
    	}
    	        ...
    	        while(test2){
    	            if(this.x[ii]==this.x[jj]){
    	                if(this.y[ii]==this.y[jj]){
                            ...
                            this.x = this.copy(xx);
                            this.y = this.copy(yy);
                            this.newAndOldIndices = this.copy(naoi);
    	                }
    	                else{
